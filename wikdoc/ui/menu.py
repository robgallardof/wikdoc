...
def _header(state: MenuState) -> None:
    ...
    if state.workspace_path:
        ...
        console.print(f"[bold]Active workspace:[/bold] {state.workspace_name or '<unnamed>'}")
        console.print(f"[bold]Path:[/bold] {state.workspace_path}")
...
def run_menu() -> None:
    ...
    while True:
        ...
        if choice == "1":
            state.workspace_path, state.workspace_name, state.local_store = _workspace_menu(
        ...
        if choice == "6":
            _packs_menu(state.workspace_path, state.workspace_name, state.local_store)
            continue
        ...
        if choice == "10":
            ...
            console.print("")
            console.print("1) Start Wikdoc API + Open WebUI")
            ...
            if sub == "3":
                ...
                console.print(
                    ...
                    "  wikdoc serve --port "
                    + str(state.rag_port)
                    + "\n  open-webui serve --port "
                    + str(state.webui_port)
                )
